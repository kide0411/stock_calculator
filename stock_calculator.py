...
def add_upper_prices():
    ...
    st.session_state.base_prices.extend([last_max + i * step for i in range(1,6)])

def add_lower_prices():
    last_min = min(st.session_state.base_prices, default=buy_price)
    step = get_price_step(last_min)
    st.session_state.base_prices = [last_min - i * step for i in range(5,0,-1)] + st.session_state.base_prices
